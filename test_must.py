...
def main():
    ...
    # for transform_type in ['defocus_blur','motion_blur','zoom_blur','contrast','brightness','saturate','jpeg_compression','wb']:
        ...
        for severity in range(1,levels):#range(1,6):
            ...
            for topic in ['meconium','fir','mir']:
                ...
                for random_seed in [100,200,300,400,500]:
                    test_data_loaders = [DataFactory.get_split_data_loader(config, topic,random_seed, split_type='train+val'),
                                    DataFactory.get_must_data_loader(config, topic)]
                    ...
                    if config.text_condition:
                        print('config.text_condition',config.text_condition)
                        val_res = linear_clf.eval_with_attn_map()
                    else:
            ...
            print(output)
    exp_df.to_csv(os.path.join(config.tb_log_dir,'must_results.csv'),index=False)
    all_exp_df.to_csv(os.path.join(config.tb_log_dir,'must_all_results.csv'),index=False)
    cases_df.to_csv(os.path.join(config.tb_log_dir,'must_case_results.csv'),index=False)
