...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds, return_attn=False):
        """
        Input
        ...
        if return_attn:  
            # cosine_sim = F.normalize(k, dim=-1) @ F.normalize(q, dim=-2)
...
class Transformer(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds, return_attn=False):
        ...
        # num_vids x num_texts x embed_dim
        if return_attn:
            attn_out, attention_weights = self.cross_attn(text_embeds, video_embeds, return_attn=True)
        else:
