...
class WBEmulator:
  def __init__(self):
    ...
    self.features = np.load('preprocess/WBAugmenter/params/features.npy')
    # mapping functions to emulate WB effects
    self.mappingFuncs = np.load('preprocess/WBAugmenter/params/mappingFuncs.npy')
    # weight matrix for histogram encoding
    self.encoderWeights = np.load('preprocess/WBAugmenter/params/encoderWeights.npy')
    # bias vector for histogram encoding
    self.encoderBias = np.load('preprocess/WBAugmenter/params/encoderBias.npy')
    ...
    #  2850K, 3800K, 5500K, 6500K, and 7500K
    # F: Fluorescent (3800K), S: Shade (7500K), T: Tungsten (2850K), C: Cloudy (6500K), D: Daylight (5500k), 
  ...
  def open_with_wb_aug(self, filename, target_dir, ind = None, target_size=None):
